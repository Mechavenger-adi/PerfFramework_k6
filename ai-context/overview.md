...
| Source files | ~75 TypeScript files across `core-engine/src/` |
...
| Path | Purpose |
|------|---------|
| `core-engine/src/cli/run.ts` | Main CLI — all commands registered here |
| `core-engine/src/index.ts` | Barrel export — public API surface |
| `config/test-plans/*.json` | Test execution shape |
| `config/runtime-settings/default.json` | Runtime behavior defaults |
