...
**Risk:** Code in `core-engine/src/utils/` compiles to `dist/utils/` and runs inside k6's goja engine, which is NOT Node.js. Accidentally importing `fs`, `path`, or any Node built-in causes k6 runtime failure.
...
**Risk:** Team folders in `scrum-suites/` are logically isolated but not enforced. Nothing prevents one team's script from importing another team's data files.

**Hidden assumption:** `PathResolver` searches all of `scrum-suites/` recursively, so a filename collision across teams could resolve to the wrong file.
