...
| Test coverage | No test framework; 2 standalone checks in `tools/` (`npm run test:merge`, `npm run validate:histogram`) |

## Knowledge System Layers

This repo's docs are a 4-layer system (design: `knowledge-architecture-proposal.md`):
- **L1** `ai_context/` — this layer: routing/rules/contracts. Front door: [FrameworkAtlas.md](../FrameworkAtlas.md).
- **L2** `engineering_docs/` — canonical EDDs (reverse-engineered from code, §4A).
- **L3** `docs/` — published user docs (derived).
- **L4** `ai_generated/` — JSON indexes (generated by `npm run docs:index`; never hand-edit).
- **`archive/`** — frozen legacy (was `.md/`); do not read or cite.

## AI Loading Strategy

```
FrameworkAtlas.md    → Front door (or the 3 below directly)
overview.md          → Always read first
module-map.md        → File-level routing table
architecture-laws.md → What you MUST NOT violate
feature_index.json   → L4 routing: feature → owning files/config/tests/EDD (≤1 read to the file)
