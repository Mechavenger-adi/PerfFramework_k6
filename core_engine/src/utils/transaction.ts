 ...
 * present.
 */
function extractScriptLocation(stack: string | undefined): string {
  ...
  for (const raw of lines) {
    ...
    for (const m of matches) {
      ...
      if (path.includes('/dist/utils/') || path.includes('\\dist\\utils\\')) continue;
      // Reject obviously bogus tokens (e.g. `https:` alone, k6 internal markers).
      if (path === 'https' || path === 'http' || path === 'native') continue;
      // Skip frames inside k6's own runtime modules.
      if (path.startsWith('k6/') || path.includes('go.k6.io')) continue;
      const lineNo = m[2];
      const col = m[3];
 ...
 */
function formatStackSnippet(stack: string | undefined, limit: number = 3): string {
  if (!stack || typeof stack !== 'string') return '';
  const lines = stack
    .split(/\r?\n/)
    .map((l) => l.trim())
...
export function transaction(name: string, fn: () => void): void {
  ...
  try {
    ...
    const gate = (globalThis as any).__k6PerfTxnGate?.();
    if (gate && gate.shouldSkipBeforeStart) {
      try { gate.onSkip?.(name); } catch { /* logging best-effort */ }
      return;
    }
    ...
    group(name, () => {
      ...
      try {
        ...
        if (behavior === 'abort_test') {
          console.error(
            `[k6-perf][transaction:${name}] ERROR: ${message}${locSuffix}\n` +
            `  → skipped remaining work in this transaction; aborting entire test (errorBehavior=abort_test)`,
          );
          exec.test.abort(`[k6-perf][transaction:${name}] ${message}${location ? ` (${location})` : ''}`);
          return;
        }
...
export function k6Check(
  ...
  if (!passed) {
    ...
    try {
      ...
      if (r && r.url) reqInfo = ` on ${r.method || 'REQ'} ${r.url}`;
    ...
    const consequence = behavior === 'abort_test' ? ' → aborting test'
      ...
      : '';
    console.error(
      `[k6-perf][check-failed] ${ctx} (VU ${vu}, iter ${iter}) check ${checksStr} failed`
