...
export function captureRequestSnapshot(
  ...
  // cap-reached warning.
  if (_snapshottedResponses.has(context.res)) return false;
 ...
 */
export function emitDeferredFailureSnapshot(
  res: any,
  ctx: { method: string; url: string; options: RequestOptions | undefined },
): boolean {
  if (!res) return false;
