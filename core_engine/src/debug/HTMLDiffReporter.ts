...
interface ReportPayload {
  ...
  k6Metrics?: K6Metrics;
  runtimeConfig?: Record<string, unknown>;
  execution?: { command?: string; options?: unknown; env?: Record<string, string> };
}
...
<body>
  ...
  private static buildPayload(results: DiffResult[], options?: ReportOptions): ReportPayload {
  ...
  private static hasMismatch(result: DiffResult): boolean {
    return (
  ...
  private static resolveStatus(
    ...
    mismatchCount: number,
    errorCount: number,
  ): ReportPayload['summary']['status'] {
    if (errorCount > 0 || missingCount > 0 || overallScore < 70) return 'broken';
    if (replayOnlyCount > 0 || mismatchCount > 0 || overallScore < 95) return 'drift';
    return 'passed';
  }

  private static findWorstTransaction(results: DiffResult[]): string {
