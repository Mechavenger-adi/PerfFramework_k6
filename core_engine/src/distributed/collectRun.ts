 ...
 */
export function collectRunDir(reportDir: string, runId: string, machineName: string, collectDir: string): string {
  const dest = path.join(path.resolve(collectDir), sharedRunFolder(runId), machineName);
  copyDirInto(reportDir, dest);
  return dest;
}
...
export function runCollect(opts: { from: string; into: string; machine: string; runId?: string }): boolean {
  ...
  const dest = collectRunDir(from, runId, opts.machine, opts.into);
