...
export class ThresholdManager {
  ...
  static apply(
    ...
    if (global) {
      ...
      if (rules.length > 0) thresholds['http_req_duration'] = rules;
    ...
    if (testPlan.transaction_slas) {
      for (const n of Object.keys(testPlan.transaction_slas)) txnNames.add(n);
    }
    for (const txn of txnNames) {
      ...
      if (rules.length > 0) thresholds[txn] = rules;
  ...
  private static flatGlobalKeys(global: GlobalSLADefinition): SLADefinition {
    const out: SLADefinition = {};
    for (const [key, value] of Object.entries(global)) {
      if (key === 'request' || key === 'transaction') continue;
  ...
  private static mergeSla(base?: SLADefinition, override?: SLADefinition): SLADefinition {
    const out: SLADefinition = {};
  ...
  static collectPercentiles(testPlan: TestPlan): string[] {
    ...
    if (testPlan.global_sla) {
      ...
      if (testPlan.global_sla.request) extractFromSLA(testPlan.global_sla.request);
      if (testPlan.global_sla.transaction) extractFromSLA(testPlan.global_sla.transaction);
    }
    if (testPlan.journey_slas) {
    ...
    if (testPlan.request_slas) {
      for (const sla of Object.values(testPlan.request_slas)) extractFromSLA(sla);
    }
