...
async function finalizeRunArtifacts(options: {
  ...
  if (transactionMetrics.hasEstimatedRows && estimatedRows.length > 0) {
    ...
    Logger.warn(
      `Pass/fail for ${estimatedRows.length} transaction(s) was estimated from native k6 check counts ` +
  ...
  if (transactionMetrics.hasEstimatedRows && estimatedRows.length > 0) {
    ...
    eventArtifacts.warnings.push(
      ErrorRuntime.buildWarningEvent(
        options.runId,
        'estimated_pass_fail',
        `Pass/fail for ${estimatedRows.length} transaction(s) is approximate — derived from native k6 ` +
        ...
        { affectedTransactions: estimatedNames } as any,
      ),
    );
  }
