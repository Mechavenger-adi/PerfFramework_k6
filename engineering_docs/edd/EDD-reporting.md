...
| **Global filter contract** | `matchesGlobalFilters` treats an **absent** scenario/transaction field as a pass, so every event must carry `journey` or the scenario filter silently no-ops on that tab. Transaction metric rows key the scenario as `journey` (not `scenario`) — the Transactions table renders it under the `journey` column with a `scenario` label. `collectFilterOptions` builds journey↔transaction `pairs` from metrics **and** errors **and** snapshots, so transactions that never produced a metric row stay selectable once a scenario is chosen. | [RunReportGenerator.ts:545-550](../../core_engine/src/reporting/RunReportGenerator.ts#L545), [:587-620](../../core_engine/src/reporting/RunReportGenerator.ts#L587) |
