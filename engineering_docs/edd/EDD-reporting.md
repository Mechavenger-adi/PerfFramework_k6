...
| **Error → snapshot join** | The Errors tab resolves each row's "View Request" via `snapshotIndexFor(row)`: `machine+vu+iteration+requestId` → `machine+vu+iteration+transaction` → `transaction` (aggregate rows only). Request ids repeat per VU (`nextRequestId()` counts per VU; converted scripts hard-code `replay.id`), so every key is machine+VU+iteration-scoped — a distributed run must never match LG-A's `req_1` to LG-B's. A row with its own vu/iteration that matches nothing gets no button rather than a foreign snapshot. | [RunReportGenerator.ts:1940-1985](../../core_engine/src/reporting/RunReportGenerator.ts#L1940), [reporting-contracts.md](../../ai_context/reporting-contracts.md) |
| **Status provenance** | The Errors table's Status column reads `row.status` from the **event**, never from the joined snapshot — sourcing it from the snapshot is what let a mis-joined row report a different request's code. `check_failed`/`http_error`/`transaction_error` events therefore carry `status`, `method`, `url`, `requestId` and `journey` from the k6 side. | [transaction.ts](../../core_engine/src/utils/transaction.ts), [request.ts](../../core_engine/src/utils/request.ts) |
| **Global filter contract** | `matchesGlobalFilters` treats an **absent** scenario/transaction field as a pass, so every event must carry `journey` or the scenario filter silently no-ops on that tab. Transaction metric rows key the scenario as `journey`; `txnRowsForRange` surfaces it under `scenario` for the table — whose column keys double as the **CSV export header**, so they must not be renamed. `collectFilterOptions` builds journey↔transaction `pairs` from metrics **and** errors **and** snapshots, so a transaction that never produced a metric row stays selectable once a scenario is chosen. | [RunReportGenerator.ts:545-550](../../core_engine/src/reporting/RunReportGenerator.ts#L545), [:587-620](../../core_engine/src/reporting/RunReportGenerator.ts#L587), [:1690-1700](../../core_engine/src/reporting/RunReportGenerator.ts#L1690) |
