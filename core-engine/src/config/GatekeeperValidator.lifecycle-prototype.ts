...
import { TestPlan, UserJourney } from '../types/TestPlanSchema.lifecycle-prototype';
...
export class GatekeeperValidator {
  ...
  private validateLifecycle(
    ...
    if (overlap.length > 0) {
      warnings.push(
        `[Journey:${journey.name}] lifecycle.init and lifecycle.end overlap: ${overlap.join(', ')}.`,
      );
    }
    ...
    if (!lifecycleExecutors.has(profile.executor)) {
      warnings.push(
        `[Journey:${journey.name}] Lifecycle prototype is reliable only for ramping-vus and per-vu-iterations. Current executor '${profile.executor}' would be best-effort only.`,
      );
    }

    if (profile.executor === 'per-vu-iterations' && (profile.iterations ?? 0) < 3) {
      failures.push(
        `[Journey:${journey.name}] Lifecycle with per-vu-iterations needs iterations >= 3 (Init + Action + End).`,
      );
    }
    ...
    if (profile.executor === 'ramping-vus') {
      ...
      if (stages.length === 0 || stages[stages.length - 1].target !== 0) {
        warnings.push(
          `[Journey:${journey.name}] Lifecycle is most accurate when ramping-vus ends with an explicit final target 0 stage.`,
        );
      }
    }
  }
