    ...
    try {
      ...
      if (schema.properties) {
        ...
        for (const [key, prop] of Object.entries(schema.properties) as any) {
          if (key === '$schema' || key === '_meta') continue;
          ...
          if (prop.type === 'object' && prop.properties) {
            ...
            for (const [nKey, nProp] of Object.entries(prop.properties) as any) {
      ...
      console.warn(`[WARN] Failed to parse schema ${file}: ${(err as Error).message}`);
    }
  }
