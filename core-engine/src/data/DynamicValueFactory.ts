...
export class DynamicValueFactory {
   ...
   */
  static timestamp(format = 'YYYY-MM-DD HH:mm:ss'): string {
    const now = new Date();
    return format
   ...
   */
  static randomEmail(prefix = 'user', domain = 'perf-test.local'): string {
   ...
   */
  static randomPhone(pattern = '07########'): string {
  ...
  static pickRandom<T>(items: T[]): T {
    if (items.length === 0) throw new Error('[DynamicValueFactory] Cannot pick from an empty array.');
