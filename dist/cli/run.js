...
const transactionCsv_1 = require("../distributed/transactionCsv");
...
program
    ...
    try {
        ...
        resolvedConfig = configManager.resolve({
            ...
            runtimeSettingsPath: opts.runtime,
            cliOverrides: {
                ...(opts.debug !== undefined ? { debugMode: opts.debug } : {}),
            },
        });
    ...
    const txnNamesForLive = runtimeEnv.K6_PERF_TRANSACTION_NAMES
        ...
        : [];
    const liveRuntime = new RuntimeConfigManager_1.RuntimeConfigManager(resolvedConfig.runtime);
 ...
 */
function buildRuntimeMetadataBlock(resolvedConfig) {
    const runtime = new RuntimeConfigManager_1.RuntimeConfigManager(resolvedConfig.runtime);
    return {
        errorBehavior: runtime.getErrorBehavior(),
        thinkTime: {
...
async function finalizeRunArtifacts(options) {
    ...
    if (k6Events.errors.some((e) => e.type === 'check_failed')) {
        const errs = eventArtifacts.errors;
        eventArtifacts.errors =
    ...
    if (checkFailures.length > 0) {
        const MAX = 20;
        logger_1.Logger.warn(`${checkFailures.length} check/request failure(s):`);
    ...
    if (breachedThresholdMetricNames.length > 0) {
        logger_1.Logger.detail(`Threshold breaches surfaced as warnings: ${breachedThresholdMetricNames.slice(0, 5).join(', ')}${breachedThresholdMetricNames.length > 5 ? ` (+${breachedThresholdMetricNames.length - 5} more)` : ''}`);
    }
    ...
    try {
        const reqCsv = (0, transactionCsv_1.findRequestCsv)(options.reportDir);
        if (reqCsv) {
            const bucketSec = timeseries.bucketSizeSeconds || 2;
            const buckets = (0, transactionCsv_1.readRequestFailByBucket)(reqCsv, bucketSec);
            for (const p of timeseries.series.overview) {
                ...
                if (rf && rf.total > 0) {
                    p.httpFailedRate = rf.failed / rf.total;
                    p.httpFailedCount = rf.failed;
                }
            }
        }
    }
    ...
    if (fs.existsSync(snapshotsFile)) {
        try {
            snapshotFiles = JSON.parse(fs.readFileSync(snapshotsFile, 'utf-8'));
        }
...
// ---------------------------------------------
const LIVE_TXN_INTERVAL_MS = 5000;
function pct(values, p) {
    ...
    const n = sorted.length;
    if (n === 1)
        return String(Math.round(sorted[0]));
...
function startLiveTransactionDisplay(metricsStreamPath, transactionNames, transactionStats, _logPath) {
                    ...
                    for (const line of buf.toString('utf-8').split('\n')) {
                        ...
                        try {
                            ...
                            if (entry.metric === 'checks') {
                                ...
                                if (!checkDisplay)
                                    continue;
                                const cs = stats.get(checkDisplay) ?? {
                            ...
                            s.values.push(v);
                            if (s.values.length > 100000)
                                s.values = s.values.slice(-50000);
...
function buildLiveTableLines(stats, transactionStats, useColor) {
    ...
    const hRow = headers.map((h, i) => i === 0 ? ` ${h.padEnd(widths[i])} ` : ` ${h.padStart(widths[i])} `).join(`${c.dim}│${c.reset}`);
 ...
 */
function parseAndFlushSnapshots(runLogPath, reportDir) {
...
function extractSnapshotPayload(line) {
    ...
    if (consoleMatch) {
        ...
        if (idx !== -1)
            return rawMessage.slice(idx + SNAPSHOT_EVENT_PREFIX.length).trim();
        return null;
    }
