...
exports.JourneyAllocator = void 0;
class JourneyAllocator {
     ...
     */
    static allocate(journeys, totalVUs) {
        if (journeys.length === 0) {
            throw new Error('[JourneyAllocator] No journeys to allocate VUs to.');
        }
        if (totalVUs < journeys.length) {
            throw new Error(`[JourneyAllocator] totalVUs (${totalVUs}) is less than number of journeys (${journeys.length}). Each journey needs at least 1 VU.`);
        }
        ...
        if (remainingVUs < weightBasedJourneys.length && weightBasedJourneys.length > 0) {
            throw new Error(`[JourneyAllocator] After explicit VU assignments (${explicitVUsTotal} VUs), only ${remainingVUs} VUs remain for ${weightBasedJourneys.length} weight-based journeys. Not enough for minimum 1 VU each.`);
        }
        // Normalize weights
        const totalWeight = weightBasedJourneys.reduce((sum, j) => sum + (j.weight ?? 1), 0);
        const allocations = journeys.map((journey) => {
        ...
        if (remainder > 0) {
            ...
            if (sorted.length > 0)
                sorted[0].allocatedVUs += remainder;
        }
        return allocations;
    }
    /** Print allocation table to console */
    static printTable(allocations) {
