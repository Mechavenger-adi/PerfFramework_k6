...
class ScenarioBuilder {
    ...
    // ---------------------------------------------
    static buildParallel(plan, metadata) {
        const scenarios = {};
    ...
    // ---------------------------------------------
    static buildSequential(plan, metadata) {
        const scenarios = {};
        let offsetSeconds = 0;
        for (const journey of plan.user_journeys) {
            const profile = journey.loadProfile ?? plan.global_load_profile;
    ...
    // ---------------------------------------------
    static buildHybrid(plan, metadata) {
    ...
    static buildScenarioEnv(plan, journey, execName, metadata, existingEnv) {
    ...
    static computePhaseEnvelope(profile, existingEnv) {
        if (existingEnv?.K6_PERF_PHASES) {
            try {
                return JSON.parse(existingEnv.K6_PERF_PHASES);
            }
            catch {
    ...
    static computeDebugPhaseEnvelope(profile) {
